...
func main() {
	...
	userRoles := []UserRole{
		...
		{UserID: "user2", TenantID: "root", ScopeName: "scope2", RoleName: "Admin"},
	}
...
func resolveUserRoles(userID, tenantID, scopeName string, userRoles []UserRole, tenants map[string]*Tenant, dag *RoleDAG) ([]Permission, error) {
...
func authorize(request Request, userRoles []UserRole, tenants map[string]*Tenant, dag *RoleDAG) bool {
	...
	permissions, err := resolveUserRoles(request.UserID, request.TenantID, request.ScopeName, userRoles, tenants, dag)
	if err != nil {
		fmt.Println(err)
		return false
	}
